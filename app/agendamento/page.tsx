...
import { getServicesWithTakenTimes } from "/app/actions/booking.ts";

const prisma = new PrismaClient();

export default async function Page() {
  // Pega a única barbearia (ou adapte para multi-barbearia)
...
const SERVICES: Service[] = [
  ...
  { id: "cabelo-barba", name: "Cabelo e Barba", price: "R$ 40", icon: <Scissors size={18} /> },
  { id: "assinatura", name: "Assinatura", price: "R$ 52,50", period: "/mês", icon: <CreditCard size={18} /> },
];

const ALL_TIMES = ["08:00", "09:00", "10:00", "11:00", "13:00", "14:00", "15:00", "16:00"];
const TAKEN_TIMES = new Set(["08:00", "09:00", "13:00"]);
...
function buildWeek(): DayOption[] {
  ...
  return Array.from({ length: 7 }, (_, i) => {
    ...
    return {
      ...
      month: d.getMonth(),
      year: d.getFullYear(),
      disabled: dow === 0 || dow === 6, // closed on weekends
    };
  });
}
...
function validateForm(data: FormData): FormErrors {
  ...
  if (digits.length < 10) errors.phone = "WhatsApp inválido";
  return errors;
}
...
function SectionCard({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="bg-white/5 border border-[#d4a017]/20 rounded p-5 mb-4">
      <h2 className="text-[10px] tracking-[0.18em] uppercase text-[#d4a017] font-bold mb-3.5">
        {title}
      </h2>
      {children}
    </div>
...
function BookingSummary({ service, day, time }: { service: Service; day: DayOption; time: string }) {
  return (
    <div className="border border-[#d4a017]/30 rounded p-4 mb-4 bg-[#d4a017]/5 flex flex-col gap-1.5">
      ...
      <div className="flex justify-between text-sm">
        ...
        <span className="font-medium">{service.name} — {service.price}{service.period ?? ""}</span>
      </div>
      <div className="flex justify-between text-sm">
...
const BookingPage = () => {
  ...
  function handleNext() {
    if (step === 3 && !selectedTime) return; // guard: time must be picked
    ...
    <div className="relative min-h-screen bg-[#0a0a0a] text-[#f5f0e8] font-sans p-9 pb-16 overflow-hidden">
      ...
      <div className="relative z-10 max-w-xl mx-auto">
        ...
        <div className="text-center mb-9">
          <Link
            href="/"
            className="inline-flex items-center gap-1 text-[11px] tracking-[0.15em] uppercase text-[#d4a017]/70 hover:text-[#d4a017] transition-colors mb-4 no-underline"
          >
            ...
            <div className="grid grid-cols-2 gap-2.5">
              {SERVICES.map((svc) => (
                <div
                  ...
                  className={`border rounded p-3.5 text-center cursor-pointer transition-all select-none ${
                    selectedService === svc.id
                      ? "bg-[#d4a017]/10 border-[#d4a017]"
                      : "border-[#d4a017]/20 hover:bg-[#d4a017]/5"
                  }`}
                >
                  <div className="w-8 h-8 bg-[#d4a017]/10 rounded-full flex items-center justify-center mx-auto mb-2 text-[#d4a017]">
                    {svc.icon}
                  </div>
                  <div className="text-[11px] tracking-widest uppercase font-medium text-[#f5f0e8]/75">
                    {svc.name}
                  </div>
                  <div className="text-sm text-[#d4a017] font-semibold mt-1">
                    {svc.price}<span className="text-xs font-normal text-[#d4a017]/60">{svc.period}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-7 gap-1.5">
              {WEEK.map((day, idx) => (
                <div
                  ...
                  className={`border rounded py-2 px-1 text-center transition-all select-none ${
                    day.disabled
                      ? "opacity-25 cursor-default"
                      : "cursor-pointer hover:bg-[#d4a017]/10"
                  } ${
                    selectedDay.num === day.num && selectedDay.month === day.month && !day.disabled
                      ? "bg-[#d4a017]/10 border-[#d4a017]"
                      : "border-[#d4a017]/15"
                  }`}
                >
        ...
        {step === 3 && (
          <SectionCard title="Escolha o horário">
            <div className="grid grid-cols-4 gap-2">
              {ALL_TIMES.map((time) => {
                const taken = TAKEN_TIMES.has(time);
                const active = selectedTime === time;
                return (
                  <div
                    key={time}
                    onClick={() => !taken && setSelectedTime(time)}
                    title={taken ? "Horário indisponível" : undefined}
                    className={`border rounded py-2.5 px-1.5 text-center text-xs font-medium tracking-wider transition-all select-none ${
                      taken
                        ? "opacity-20 cursor-default line-through"
                        : active
                        ? "bg-[#d4a017]/10 border-[#d4a017] text-[#f5f0e8] cursor-pointer"
                        : "border-[#d4a017]/15 text-[#f5f0e8]/70 hover:text-[#f5f0e8] hover:border-[#d4a017] cursor-pointer"
                    }`}
                  >
                    {time}
                  </div>
            ...
            {!selectedTime && (
              <p className="text-[11px] text-[#d4a017]/60 mt-3">Selecione um horário para continuar.</p>
            )}
          </SectionCard>
        )}
              ...
              <div className="flex flex-col gap-2.5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  <div className="flex flex-col gap-1">
                    <input
                      className={`bg-white/5 border text-[#f5f0e8] p-2.5 px-3.5 text-sm rounded outline-none focus:border-[#d4a017]/50 placeholder:text-[#f5f0e8]/30 transition-colors ${errors.name ? "border-red-500/60" : "border-[#f5f0e8]/20"}`}
                      type="text"
                  ...
                  <div className="flex flex-col gap-1">
                    <input
                      className={`bg-white/5 border text-[#f5f0e8] p-2.5 px-3.5 text-sm rounded outline-none focus:border-[#d4a017]/50 placeholder:text-[#f5f0e8]/30 transition-colors ${errors.phone ? "border-red-500/60" : "border-[#f5f0e8]/20"}`}
                      type="tel"
                      placeholder="WhatsApp (ex: 83 9 9999-9999)"
                      value={form.phone}
                ...
                <button
                  ...
                  className="w-full bg-[#d4a017] hover:bg-[#e6b420] active:scale-[0.98] text-[#0a0a0a] py-3.5 px-4 text-xs font-bold tracking-[0.18em] uppercase rounded transition-all mt-1.5"
                >
        ...
        <div className={`flex mt-2 gap-3 ${step > 1 ? "justify-between" : "justify-end"}`}>
          {step > 1 && (
            <button
              onClick={handleBack}
              className="text-[11px] tracking-widest uppercase text-[#f5f0e8]/40 hover:text-[#f5f0e8]/70 transition-colors flex items-center gap-1"
            >
